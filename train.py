...
#wandb
parser.add_argument('--wbProj', type=str, default='aanet+', help="Wandb project name")
parser.add_argument('--wbRunName', type=str, default='aanet+', help="Wandb run name")
...
def main():
    ...
    if args.wandb == 1:
        import wandb
        wandb.init(project=args.wbProj, entity="KumailRaza")
        wandb.run.name = args.wbRunName
        wandb.run.save()
        # wandb.watch(model)
    else:
        wandb = None
    ...
    train_model = model.Model(args, logger,  optimizer, aanet, device, start_iter, start_epoch,
                                   best_epe=best_epe, best_epoch=best_epoch, wb_logger=wandb)
