...
class Model(object):
    def __init__(self, args, logger, optimizer, aanet, device, start_iter=0, start_epoch=0,
                 best_epe=None, best_epoch=None, wb_logger=None):
        self.args = args
        self.logger = logger
    ...
    def train(self, train_loader):
        ...
        if wb_logger is not None:
            wb_logger.log({"base_lr": base_lr, "offset_lr": offset_lr, "epoch": self.epoch + 1})
        ...
        for i, sample in enumerate(train_loader):
            ...
            if self.num_iter % args.summary_freq == 0:
                ...
                save_images(self.train_writer, wb_logger, 'train', img_summary, self.num_iter)
                ...
                if wb_logger is not None:
                    wb_logger.log({'train/epe': epe.item(), 'train/disp_loss': disp_loss.item(), 'train/total_loss': total_loss.item()})
                ...
                for s in range(len(pyramid_loss)):
                    ...
                    if wb_logger is not None:
                        wb_logger.log({save_name: save_value, 'train/disp_loss': disp_loss.item(),
                               'train/total_loss': total_loss.item()})

                d1 = d1_metric(pred_disp, gt_disp, mask)
                self.train_writer.add_scalar('train/d1', d1.item(), self.num_iter)
                if wb_logger is not None:
                    wb_logger.log({'train/D1': d1.item()})
                ...
                if wb_logger is not None:
                    wb_logger.log({'train/thres1': thres1.item(), 'train/thres2': thres2.item(), 'train/thres3': thres3.item()})
    ...
    def validate(self, val_loader):
        ...
        for i, sample in enumerate(val_loader):
            ...
            if not args.evaluate_only:
                if i in [num_samples // 4, num_samples // 2, num_samples // 4 * 3]:
                    ...
                    save_images(self.train_writer, wb_logger, 'val' + str(val_count), img_summary, self.epoch)
                    val_count += 1
        ...
        if not args.evaluate_only:
            ...
            if wb_logger is not None:
                wb_logger.log({'val/epe': mean_epe, 'epoch': self.epoch,
                       'val/d1': mean_d1,
                       'val/thres1': mean_thres1,
                       'val/thres2': mean_thres2,
                       'val/thres3': mean_thres3
                       })
