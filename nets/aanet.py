...
class AANet(nn.Module):
    ...
    def add_residual(self, disp, res):
        assert len(disp) == len(res)
        output = []
        for d, r in zip(disp, res):
            assert d.size() == r.size()
            output.append(d + r)
        assert len(disp) == len(output)
        return output
    ...
    def forward(self, left_img, right_img):
        ...
        # print([len(j) for j in features_x])
        # for j in features_x:
        #     print([k.size() for k in j])
        # exit(0)
        ...
        #                                                disparity_pyramid[-1])
        # print("==> disp_pyramid_refined: ", [j.size() for j in disparity_pyramid])
        # print("==> max, min disps: ", [[j.max(), j.min()] for j in disparity_pyramid])
        # exit(0)
        return disparity_pyramid
